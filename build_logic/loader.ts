	...
	if (!STATE.loaded) {
		...
		for await (const dir_entry of Deno.readDir("./loaders")) {
			if (dir_entry.isFile && dir_entry.name.endsWith(".ts")) {
				loaders.push(dir_entry.name.substring(0, dir_entry.name.length - 3));
			}
		}

		STATE.loaders = await Promise.all(loaders.map(async (id) => {
			const loader = (await import(`../loaders/${id}.ts`))["default"];
			loader.id = id;
			return loader;
		}));
